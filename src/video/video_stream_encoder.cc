...
const int64_t kParameterUpdateIntervalMs = 100;
...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  //sandy:Mp-WebRTC: For MP-WebRTC I have included the maximum limit of twice
...
void VideoStreamEncoder::OnFrame(const VideoFrame& video_frame) {
  ...
  encoder_queue_.PostTask(
      [this, incoming_frame, post_time_us, log_stats]() {
        ...
        if (log_stats) {
          RTC_LOG(LS_INFO) << "sandystats Number of frames: captured "
                           << captured_frame_count_
...
void VideoStreamEncoder::EncodeVideoFrame(const VideoFrame& video_frame,
  ...
  // else{
  //   RTC_LOG(INFO)<<"sandystats: Encoded video\t"<<out_frame.width()<<" * " <<out_frame.height()<<"\t Encoded info: impl name\t"<<
  //               encoder_info_.implementation_name<<"\t input framerate\t"<<GetInputFramerateFps()<<"\t bitrate of encoding\t"<<
...
void VideoStreamEncoder::OnBitrateUpdated(DataRate target_bitrate,
  ...
  RTC_LOG(INFO) << "sandystats OnBitrateUpdated, bitrate " << target_bitrate.kbps()
                      << " stable bitrate = " << stable_target_bitrate.kbps()
                      << " link allocation bitrate = " << link_allocation.kbps()
