...
PacketReceiver::DeliveryStatus FakeNetworkPipe::MpDeliverPacket(
    MediaType media_type,
    rtc::CopyOnWriteBuffer packet,
...
void FakeNetworkPipe::DeliverNetworkPacket(NetworkPacket* packet) {
  ...
  //sandy: This part should never be called so I am putting fake error
  RTC_DCHECK(1==0);
  if (transport) {
    ...
    if (packet->is_rtcp()) {
      ...
      transport->SendRtcp(packet->data(), packet->data_length(),1);
    } else {
