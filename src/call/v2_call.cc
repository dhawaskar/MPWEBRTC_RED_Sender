...
const int MpReorderingCount_=20; //sandy:Reordering buffer for RTP packets into receiver controller
const int MpReorderingwaitCount_=5;
// const int MpMediaReorderingCount_=15; //sandy:Reordering buffer for RTP packets into video Media controller
// const int MpMediaReorderingwaitCount=10; //sandy: How long should I wait if packet is lost?
uint16_t MpMediaseqnext=0;
// uint16_t MpMediaseqlastlost=0;
uint16_t Mpseqlastlost=0;
...
void Call::NotifyBweOfReceivedPacket(const RtpPacketReceived& packet,
  ...
  if (media_type == MediaType::VIDEO ||
    ...
    //Here you cannot send packets you have to make sure they are in order.
    ...
    if(header.sequenceNumber>header.extension.mpflowseqnum && header.extension.hasmpflowseqnum && 
      ...
      RTC_LOG(INFO)<<"sandystatsofo let us insert the transport feedback "<<header.extension.transportSequenceNumber << 
      " MP: "<<header.extension.mptransportSequenceNumber;
      Mpcount_++;
      if(Mpcount_>MpReorderingCount_){
        Mpseqnext=Mpseq_;
        RTC_LOG(INFO)<<"sandystatsofo let us send the transport feedback "<<Mpcount_<<" start= "<<Mpseq_<<" cur "<<Mpseqnext;
        int i;
        for(i=0;i<MpReorderingCount_;i++){
          ...
          if(it!=Mphistory_.end()){
            ...
            RTC_LOG(INFO)<<"sandystatsofo sending transport feedback "<<Mpseqnext;
            RTPHeader Mpheader;
