    ...
    pacer_queue_update_task_ = RepeatingTaskHandle::DelayedStart(
        task_queue_.Get(), kPacerQueueUpdateInterval, [this]() {
          ...
          //sandy: Change #1 double the Queue size
            control_handler_->SetPacerQueue(expected_queue_time*2);
          }else{
...
void RtpTransportControllerSend::PostUpdates(NetworkControlUpdate update,NetworkControlUpdate update2) {
  ...
  if(mpcollector_->MpGetScheduler().find("window")!=std::string::npos&&mpcollector_->MpISsecondPathOpen()){
    window_scheduler=1;
    //sandyasymmetry rates
    // if(mpcollector_->MpGetRemoteAsymmetryRR1() && update.target_rate){
    //   DataRate rate1=DataRate::KilobitsPerSec(mpcollector_->MpGetRemoteAsymmetryRR1());
    //   DataRate rate2=update.target_rate->target_rate;
    //   update.target_rate->target_rate=std::min(update.target_rate->target_rate,rate1);
    //   RTC_LOG(INFO)<<"sandyasymmetry changing the rate 1 "<< 
    //   " origianl R1 "<<rate2.bps()<<" new R1 "<<update.target_rate->target_rate.bps()<<" bps "; 
      
    // }
    // if(mpcollector_->MpGetRemoteAsymmetryRR2() && update2.target_rate){
    //  DataRate rate1=DataRate::KilobitsPerSec(mpcollector_->MpGetRemoteAsymmetryRR2());
    //   DataRate rate2=update2.target_rate->target_rate;
    //   update2.target_rate->target_rate=std::min(update2.target_rate->target_rate,rate1);
    //   RTC_LOG(INFO)<<"sandyasymmetry changing the rate 2 "<< 
    //   " origianl R2 "<<rate2.bps()<<" new R2 "<<update2.target_rate->target_rate.bps()<<" bps "; 
    // }
    //sandy: Set the each path encoding rate by limit of the maximum encding rate
   
    // if(update.target_rate){
    //  DataRate rate1=DataRate::KilobitsPerSec(mpcollector_->MpGetNumberOfCameraStreams()*10000);
    //  DataRate rate2=update.target_rate->target_rate;
    //  update.target_rate->target_rate=std::min(update.target_rate->target_rate,rate1);
    //   RTC_LOG(INFO)<<"sandyasymmetry changing the rate 1 "<< 
    //   " origianl R1 "<<rate2.bps()<<" new R1 "<<update.target_rate->target_rate.bps()<<" bps "; 
  ...
  mpcollector_->MpISsecondPathOpen()){//sandy: If the primary path has received the signal
    ...
    if (update.target_rate && update2.target_rate) {
      ...
      update2.target_rate->network_estimate.loss_rate_ratio<<" Pacer rate 1 "<<pp_pacer_config->data_rate().bps()<<" Pacer rate 2  "<<sp_pacer_config->data_rate().bps();
      ...
      if(sp_target_rate){
        ...
        sp_target_rate->network_estimate.loss_rate_ratio<<" Pacer rate 1 "<<pp_pacer_config->data_rate().bps()<<" Pacer rate 2  "<<sp_pacer_config->data_rate().bps();
