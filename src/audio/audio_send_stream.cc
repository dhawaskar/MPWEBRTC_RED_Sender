...
AudioSendStream::ExtensionIds AudioSendStream::FindExtensionIds(
  ...
  for (const auto& extension : extensions) {
    ...
    } else if (extension.uri == RtpExtension::ksandyUri){ //Mp-webRTC
      ids.abs_sandy_val = extension.id;
    } else if (extension.uri == RtpExtension::kMpFlowSeqNumUri){//Mp-webRTC
      ids.mpflowseqnum = extension.id;
    } else if (extension.uri == RtpExtension::kMpTransportSequenceNumberUri) {
      ids.mp_transport_sequence_number = extension.id;
    }
  }
  return ids;
}
...
void AudioSendStream::ConfigureStream(
  ...
  if (first_time || new_ids.abs_sandy_val != old_ids.abs_sandy_val) {
      rtp_rtcp_module_->DeregisterSendRtpHeaderExtension(
          kRtpExtensionsandy);
      if (new_ids.abs_sandy_val) {
        rtp_rtcp_module_->RegisterRtpHeaderExtension(sandy::kUri,
                                                     new_ids.abs_sandy_val);
      }
  }
  //sandy:mp-Webrtc
  if (first_time || new_ids.mpflowseqnum != old_ids.mpflowseqnum) {
      rtp_rtcp_module_->DeregisterSendRtpHeaderExtension(
          kRtpExtensionMpFlowSeqNum);
      if (new_ids.mpflowseqnum) {
        rtp_rtcp_module_->RegisterRtpHeaderExtension(MpFlowSeqNum::kUri,
                                                     new_ids.mpflowseqnum);
      }
  }

  //sandy:mp-Webrtc
  if (first_time || new_ids.mp_transport_sequence_number != old_ids.mp_transport_sequence_number) {
      rtp_rtcp_module_->DeregisterSendRtpHeaderExtension(
          kRtpExtensionMpTransportSequenceNumber);
      if (new_ids.mp_transport_sequence_number) {
        rtp_rtcp_module_->RegisterRtpHeaderExtension(MpTransportSequenceNumber::kUri,
                                                     new_ids.mp_transport_sequence_number);
      }
  }
