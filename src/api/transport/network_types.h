...
struct SentPacket {
  ...
  int64_t mp_sequence_number;//sandy
  int pathid;//sandy
...
struct TransportLossReport {
  ...
  //sandy: Multipath code
  // uint64_t packets_lost_delta_p = 0;
  // uint64_t packets_received_delta_p = 0;
  // uint64_t packets_lost_delta_s = 0;
  // uint64_t packets_received_delta_s = 0;
  int pathid=-1;
};
...
struct TargetTransferRate {
  ...
  NetworkEstimate network_estimate_second;
  DataRate target_rate = DataRate::Zero();
  DataRate target_rate_second=DataRate::Zero();
  DataRate stable_target_rate = DataRate::Zero();
  DataRate stable_target_rate_second = DataRate::Zero();
  double cwnd_reduce_ratio = 0;
  double cwnd_reduce_ratio_second = 0;
};
...
struct NetworkControlUpdate {
  ...
  absl::optional<DataSize> congestion_window_second;
  absl::optional<PacerConfig> pacer_config;
  std::vector<ProbeClusterConfig> probe_cluster_configs;
  std::vector<ProbeClusterConfig> probe_cluster_configs_second;
  absl::optional<TargetTransferRate> target_rate;
  absl::optional<TargetTransferRate> target_rate_second;
};
