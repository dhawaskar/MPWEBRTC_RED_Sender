...
class NackModule2 final {
 public:
  ...
  NackModule2(TaskQueueBase* current_queue,
              ...
              TimeDelta update_interval = kUpdateInterval,int pathid=-1);
  ~NackModule2();

  int OnReceivedPacket(uint16_t seq_num, bool is_keyframe,int pathid);
  int OnReceivedPacket(uint16_t seq_num, bool is_keyframe, bool is_recovered,int pathid);
