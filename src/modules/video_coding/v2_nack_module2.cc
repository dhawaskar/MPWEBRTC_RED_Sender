...
int NackModule2::OnReceivedPacket(uint16_t seq_num,uint16_t mp_seq_num,
  ...
  if(!initialized_mp_&& mp_seq_num!=seq_num && mp_seq_num>0){//sandy: This indicates Mp_webRTC is enabled
    //RTC_LOG(INFO)<<"sandy the mp-webrtc is enabled with difference in seq and mp_seq\n";
  ...
  RTC_LOG(INFO)<<"sandy:Received the packet "<<seq_num<<" path= "<<pathid<< 
  ...
  if(initialized_mp_ &&  pathid!=2 && mp_seq_num>0){
    ...
    //RTC_LOG(INFO)<<"Sandy added the packet to the list of primary path mp="<<mp_seq_num<< 
    ...
    // RTC_LOG(INFO)<<"Sandy added the packet to the list of secondary path mp="<<mp_seq_num<< 
  ...
  if (!nack_batch.empty()) {
    ...
    RTC_LOG(INFO)<<"sandy sending the primary nack list "<<nack_batch.size();
  ...
  if (!nack_batch.empty()) {
    // This batch of NACKs is triggered externally; the initiator can
    // batch them with other feedback messages.
    RTC_LOG(INFO)<<"sandy sending the secondary nack list "<<nack_batch.size();
...
void NackModule2::AddPacketsToNack(uint16_t seq_num_start, uint16_t seq_num_end, 
  ...
  // RTC_LOG(INFO)<<"sandy adding packet to nack received"<<seq_num_start<<" from pathid ="<<pathid<< 
  // " mp_seq "<<mp_seq_num_end<<"\n";
  ...
  if( pathid!=2 && mp_seq_num_start>0){//sandy:Primary path nack handler
      ...
      // RTC_LOG(INFO)<<"sandy the primary NACK gap is "<<num_new_nacks<<" s: "<< 
      ...
      num_new_nacks=ForwardDiff(mp_seq_num_start,mp_seq_num_end);
      // RTC_LOG(INFO)<<"sandy the secondary NACK gap is "<<num_new_nacks<<" s: "<<mp_seq_num_start  
      //  <<" e: "<<mp_seq_num_end<<" last mp= "<<newest_seq_num_s_<<" last seq= " 
      // <<seq_num_start-1<<" cur seq= "<<seq_num_end<<"\n";
  }
  else{
    num_new_nacks=ForwardDiff(seq_num_start,seq_num_end);
    // RTC_LOG(INFO)<<"sandy the default NACK gap "<<num_new_nacks<<" s: "<< 
  ...
  // RTC_LOG(INFO) << "sandy: NACK list size "<<pathid<< 
...
void NackModule2::InsertPacketNackPrimary(uint16_t original_seq,uint16_t mp_seq_num,
     ...
     auto it=nack_list_p_.find(original_seq);
      if(it!=nack_list_p_.end()){
        ...
        // RTC_LOG(INFO)<<"sandy the starting seq for primary = "<<original_seq<<" the ending seq_num_end= "<< 
        // seq_num_end<<" mp_seq = "<<mp_seq_num<<"\n";
      }else{
        return;
      }

      // RTC_LOG(INFO)<<"sandy the insertion logic primary"<<original_seq<<" from pathid ="<<pathid<<" mp_seq: " 
      ...
      for (uint16_t seq_num =original_seq; seq_num !=seq_num_end; (seq_num+=2)) {//sandy: +1 is for Mp-WebRTC
        ...
        // RTC_LOG(INFO)<<"sandy insterted into primary nack list "<<seq_num<<" from pathid ="<<pathid<< 
...
void NackModule2::InsertPacketNackSecondary(uint16_t original_seq,uint16_t mp_seq_num,
    ...
    auto it=nack_list_s_.find(original_seq);
      if(it!=nack_list_s_.end()){
        original_seq=it->second+2;
        // RTC_LOG(INFO)<<"sandy the starting seq for secondary = "<<original_seq<<" the ending seq_num_end= "<< 
        // seq_num_end<<" mp_seq = "<<mp_seq_num<<"\n";
      }else{
        return;
      }

    // RTC_LOG(INFO)<<"sandy the insertion logic secondary"<<original_seq<<" from pathid ="<<pathid<<" mp_seq: " 
    //     <<mp_seq_num<<" : "<<mp_seq_num_<<"\n";
    ...
    for (uint16_t seq_num = original_seq; seq_num !=seq_num_end;(seq_num+=2)) {//sandy: +1 is for Mp-WebRTC
        ...
        nack_list_[seq_num] = nack_info;//sandy: Adding to the NACK list
        // RTC_LOG(INFO)<<"sandy insterted into secondary nack list "<<seq_num<<" from pathid ="<< 
...
void NackModule2::InsertPacketNack(uint16_t seq_num_start,uint16_t mp_seq_num,
                                   uint16_t seq_num_end,int pathid){
    // RTC_LOG(INFO)<<"sandy the insertion logic "<<seq_num_end<<" from pathid ="<<pathid<<" mp_seq: " 
    //     <<mp_seq_num<<" : "<<mp_seq_num_<<"\n";
    ...
    for (uint16_t seq_num = seq_num_start; seq_num != seq_num_end; ++seq_num) {//sandy: +1 is for Mp-WebRTC
        ...
        nack_list_[seq_num] = nack_info;//sandy: Adding to the NACK list
        // RTC_LOG(INFO)<<"sandy insterted into default nack list "<<seq_num<<" from pathid ="<<pathid<<  
...
std::vector<uint16_t> NackModule2::GetNackBatch(NackFilterOptions options,int pathid) {
  ...
  while (it != nack_list_.end()) {
    ...
    if (delay_timed_out && ((consider_seq_num && nack_on_seq_num_passed) ||
      ...
      if(pathid==it->second.pathid_){
        //RTC_LOG(INFO)<<"sandy the pathid for the batch prepared is "<<it->second.pathid_<<"\n";
