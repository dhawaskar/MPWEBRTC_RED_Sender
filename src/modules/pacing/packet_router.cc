...
void PacketRouter::SendPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  if (packet->HasExtension<TransportSequenceNumber>() && packet->HasExtension<MpTransportSequenceNumber>() ) { //sandy: Note: The Transport Feed Back is only sent for Video file not for audio packets. Hence transport feed back sequencec number
    //is always smaller than the total number of packets sent
    if(packet->packet_type()==RtpPacketMediaType::kRetransmission){
      // RTC_LOG(INFO)<<"sandy the packet is kRetransmission in packer router path "<<packet->flowid<<":"<<packet->flowseq<<":"<<  
      // *(packet->GetExtension<TransportSequenceNumber>())<<":"<<*(packet->GetExtension<MpTransportSequenceNumber>())<<"\n";
    ...
    if(!mpcollector_->MpISsecondPathOpen() && transport_seq_!=transport_seq_p_){
      transport_seq_p_=transport_seq_;
      packet->SetExtension<MpTransportSequenceNumber>((transport_seq_p_) & 0xFFFF);
    }
  }
...
bool PacketRouter::SendCombinedRtcpPacket(
    std::vector<std::unique_ptr<rtcp::RtcpPacket>> packets,int pathid) {
  MutexLock lock(&modules_mutex_);
  ...
  for (RtpRtcpInterface* rtp_module : send_modules_list_) {
    ...
    rtp_module->SendCombinedRtcpPacket(std::move(packets),pathid);
    return true;
  }
  ...
  rtcp_sender->SendCombinedRtcpPacket(std::move(packets),pathid);
  return true;
}
