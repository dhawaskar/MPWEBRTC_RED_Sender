...
class PacketRouter : public RemoteBitrateObserver,
 ...
 public:
  ...
  bool SendCombinedRtcpPacket(
      std::vector<std::unique_ptr<rtcp::RtcpPacket>> packets,int pathid) override;
 ...
 private:
  ...
  uint64_t transport_seq_p_ RTC_GUARDED_BY(modules_mutex_);
  uint64_t transport_seq_s_ RTC_GUARDED_BY(modules_mutex_);
