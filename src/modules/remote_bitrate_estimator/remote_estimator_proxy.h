...
class RemoteEstimatorProxy : public RemoteBitrateEstimator {
 public:
  ...
  // void OnRttUpdate(int64_t avg_rtt_ms, int64_t max_rtt_ms,int pathid) override {}//sandy: This is updated after video/call_stats2.cc file is updated. I.e RTT is updated
 ...
 private:
  ...
  int64_t last_process_time_ms_s_;
  int64_t last_process_time_ms_p_;
  ...
  //Sandy: MpWebRTC we will define the start sequence holder and arrive time array for both primary and secondary path.
  absl::optional<int64_t> periodic_window_start_seq_p_ RTC_GUARDED_BY(&lock_);
  // Map unwrapped seq -> time.
  std::map<int64_t, int64_t> packet_arrival_times_p_ RTC_GUARDED_BY(&lock_);
  uint8_t feedback_packet_count_p_ RTC_GUARDED_BY(&lock_);

  absl::optional<int64_t> periodic_window_start_seq_s_ RTC_GUARDED_BY(&lock_);
  // Map unwrapped seq -> time.
  std::map<int64_t, int64_t> packet_arrival_times_s_ RTC_GUARDED_BY(&lock_);
  uint8_t feedback_packet_count_s_ RTC_GUARDED_BY(&lock_);
