...
ModuleRtpRtcpImpl::RtpSenderContext::RtpSenderContext(
    ...
    : packet_history_p(config.clock, config.enable_rtx_padding_prioritization),
      ...
      packet_sender(config, &packet_history_p),
...
void ModuleRtpRtcpImpl::Process() {
  ...
  if (rtcp_sender_.Sending()) {
    ...
    // |process_rtt| first.
    if (process_rtt &&
        rtcp_receiver_.LastReceivedReportBlockMs(1) > last_rtt_process_time_) {
      ...
      for (std::vector<RTCPReportBlock>::iterator it = receive_blocks.begin();
        ...
        int64_t rtt = 0;
        rtcp_receiver_.RTT(it->sender_ssrc, &rtt, NULL, NULL, NULL,1);
    ...
    if (process_rtt) {
      int64_t rtt_ms;
      if (rtt_stats_ && rtcp_receiver_.GetAndResetXrRrRtt(&rtt_ms,1)) {
...
void ModuleRtpRtcpImpl::OnPacketsAcknowledged(
  ...
  RTC_DCHECK(rtp_sender_);
  rtp_sender_->packet_history_p.CullAcknowledgedPackets(sequence_numbers);
}
...
int32_t ModuleRtpRtcpImpl::RTT(const uint32_t remote_ssrc,
  ...
  int32_t ret = rtcp_receiver_.RTT(remote_ssrc, rtt, avg_rtt, min_rtt, max_rtt,pathid);
  if (rtt && *rtt == 0) {
...
int32_t ModuleRtpRtcpImpl::RemoteRTCPStat(
  ...
  return rtcp_receiver_.StatisticsReceived(receive_blocks,1);
}

std::vector<ReportBlockData> ModuleRtpRtcpImpl::GetLatestReportBlockData(int pathid)
    const {
  return rtcp_receiver_.GetLatestReportBlockData(1);
}
...
bool ModuleRtpRtcpImpl::StorePackets(int pathid) const {
...
void ModuleRtpRtcpImpl::SendCombinedRtcpPacket(
  ...
  rtcp_sender_.SendCombinedRtcpPacket(std::move(rtcp_packets),pathid);//sandy: Dont know here yet so just -1 for MpWebRTC path id
}
...
void ModuleRtpRtcpImpl::OnReceivedNack(
  ...
  if (!StorePackets(1) || nack_sequence_numbers.empty()) {
    return;
  }
  // Use RTT from RtcpRttStats class if provided.
  int64_t rtt = rtt_ms();
  ...
  rtp_sender_->packet_generator.OnReceivedNack(nack_sequence_numbers, rtt,-1);
}
