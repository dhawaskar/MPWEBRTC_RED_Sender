...
RtpHeaderExtensionMap RegisterSupportedExtensions(
  ...
  for (const auto& extension : rtp_header_extensions) {
    ...
    } else if (extension.uri == MpTransportSequenceNumber::kUri) {
      map.Register<MpTransportSequenceNumber>(extension.id);//sandy
...
std::vector<std::unique_ptr<RtpPacketToSend>> FlexfecSender::GetFecPackets() {
  ...
  for (const auto* fec_packet : ulpfec_generator_.generated_fec_packets_) {
    ...
    fec_packet_to_send->ReserveExtension<sandy>();//mp-webrtc
    //fec_packet_to_send->ReserveExtension<MpFlowID>();//mp-webrtc
    fec_packet_to_send->ReserveExtension<MpFlowSeqNum>();//mp-webrtc
    fec_packet_to_send->ReserveExtension<MpTransportSequenceNumber>();//mp-webrtc
