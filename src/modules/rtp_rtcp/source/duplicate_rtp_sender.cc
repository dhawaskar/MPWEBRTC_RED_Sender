...
int64_t worst_path_time=0;
int64_t sandy_mp_start_time=0;
long worst_path_bytes=0;
namespace webrtc {
...
void RTPSender::MPTrafficSplitImplementation(
  ...
  //sandy: Copy all the packets once
  ...
  // RTC_LOG(INFO)<<"sandydup original packets size "<<packets.size()<<" copied packets size "<<copy_packets.size();
  int64_t now_ms = clock_->TimeInMilliseconds();
  ...
  }else if((mpcollector_->MpGetScheduler().find("window")!=std::string::npos)&&mpcollector_->MpISsecondPathOpen()){
    
    for (auto& packet : packets) {
      // RTC_LOG(INFO)<<"sandydup preparing packets to be on second path";
      packet->subflow_id=2;
      packet->subflow_seq=sequence_number_s_++;
      packet->SetExtension<MpFlowSeqNum>(packet->subflow_seq);
      packet->SetExtension<sandy>(packet->subflow_id);
  ...
  paced_sender_->EnqueuePackets(std::move(packets));//Traffic is only in one single path
  paced_sender_->EnqueuePackets(std::move(copy_packets));
  return;  
}
