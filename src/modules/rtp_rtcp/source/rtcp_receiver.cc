...
RTCPReceiver::RTCPReceiver(const RtpRtcpInterface::Configuration& config,
    ...
    : clock_(config.clock),
      ...
      remote_ssrc_(0),
      remote_sender_rtp_time_p_(0),
      remote_sender_rtp_time_s_(0),
      xr_rrtr_status_(false),
      xr_rr_rtt_ms_(0),
      oldest_tmmbr_info_ms_(0),
      last_received_rb_ms_p_(0),
      last_received_rb_ms_s_(0),
...
void RTCPReceiver::IncomingPacket(rtc::ArrayView<const uint8_t> packet) {
  ...
  //RTC_LOG(INFO)<<"*** sandystats path id of RTCP packet received start ***\n";
  CommonHeader rtcp_block;
    for (const uint8_t* next_block = packet.begin(); next_block != packet.end();next_block =  
      rtcp_block.NextPacket()) {
  ...
  if (!ParseCompoundPacket(packet, &packet_information)){
    //RTC_LOG(INFO)<<"sandy dropping the RTCP packets";
    return;
  }
  TriggerCallbacksFromRtcpPacket(packet_information);
}

int64_t RTCPReceiver::LastReceivedReportBlockMs(int pathid) const {
  rtc::CritScope lock(&rtcp_receiver_lock_);
  if(pathid!=2)
    return last_received_rb_ms_p_;
  else
    return last_received_rb_ms_s_;
}

void RTCPReceiver::SetRemoteSSRC(uint32_t ssrc) {
  rtc::CritScope lock(&rtcp_receiver_lock_);
  // New SSRC reset old reports.
  last_received_sr_ntp_p_.Reset();
  last_received_sr_ntp_s_.Reset();
  remote_ssrc_ = ssrc;
}
...
int32_t RTCPReceiver::RTT(uint32_t remote_ssrc,
                          ...
                          int64_t* avg_rtt_ms,
                          int64_t* min_rtt_ms,
                          int64_t* max_rtt_ms,int pathid) const {
  rtc::CritScope lock(&rtcp_receiver_lock_);


  if(pathid!=2){//sandy: Primary path RTT
    auto it = received_report_blocks_p_.find(main_ssrc_);
    if (it == received_report_blocks_p_.end())
    return -1;
    ...
    if (last_rtt_ms){
      *last_rtt_ms = report_block_data->last_rtt_ms();
      ////RTC_LOG(INFO)<<"sandyrtt the primary path rtt is "<<*last_rtt_ms<<"\n";
    }
  ...
  }else if(pathid==2){//sandy: Secondary path RTT
    auto it = received_report_blocks_s_.find(main_ssrc_);
    if (it == received_report_blocks_s_.end())
      return -1;
    ...
    if (last_rtt_ms){
      *last_rtt_ms = report_block_data->last_rtt_ms();
      ////RTC_LOG(INFO)<<"sandyrtt the secondary path rtt is "<<*last_rtt_ms<<"\n";
    }
...
//sandy: you have to implement this too called from rtc_rtcp_imple2.cc
bool RTCPReceiver::GetAndResetXrRrRtt(int64_t* rtt_ms,int pathid) {
  RTC_DCHECK(rtt_ms);
  rtc::CritScope lock(&rtcp_receiver_lock_);
  if (xr_rr_rtt_ms_ == 0) {
    return false;
...
bool RTCPReceiver::NTP(uint32_t* received_ntp_secs,
                       ...
                       uint32_t* rtcp_timestamp,int pathid) const {
  rtc::CritScope lock(&rtcp_receiver_lock_);
  if(pathid!=2){
    if (!last_received_sr_ntp_p_.Valid())
    return false;

    // NTP from incoming SenderReport.
    if (received_ntp_secs)
      *received_ntp_secs = remote_sender_ntp_time_p_.seconds();
    if (received_ntp_frac)
      *received_ntp_frac = remote_sender_ntp_time_p_.fractions();

    // Rtp time from incoming SenderReport.
    if (rtcp_timestamp)
      *rtcp_timestamp = remote_sender_rtp_time_p_;

    // Local NTP time when we received a RTCP packet with a send block.
    if (rtcp_arrival_time_secs)
      *rtcp_arrival_time_secs = last_received_sr_ntp_p_.seconds();
    if (rtcp_arrival_time_frac)
      *rtcp_arrival_time_frac = last_received_sr_ntp_p_.fractions();

  }else{

    if (!last_received_sr_ntp_s_.Valid())
    return false;

    // NTP from incoming SenderReport.
    if (received_ntp_secs)
      *received_ntp_secs = remote_sender_ntp_time_s_.seconds();
    if (received_ntp_frac)
      *received_ntp_frac = remote_sender_ntp_time_s_.fractions();

    // Rtp time from incoming SenderReport.
    if (rtcp_timestamp)
      *rtcp_timestamp = remote_sender_rtp_time_s_;

    // Local NTP time when we received a RTCP packet with a send block.
    if (rtcp_arrival_time_secs)
      *rtcp_arrival_time_secs = last_received_sr_ntp_s_.seconds();
    if (rtcp_arrival_time_frac)
      *rtcp_arrival_time_frac = last_received_sr_ntp_s_.fractions();    
...
int32_t RTCPReceiver::StatisticsReceived(
    std::vector<RTCPReportBlock>* receive_blocks,int pathid) const {

  RTC_DCHECK(receive_blocks);
  rtc::CritScope lock(&rtcp_receiver_lock_);
  if(pathid!=2){//sandy: primary received_report_blocks_p_
    for (const auto& reports_per_receiver : received_report_blocks_p_)
      for (const auto& report : reports_per_receiver.second)
        receive_blocks->push_back(report.second.report_block());
  }else{ //secondaryreceived_report_blocks_s_ for
    for (const auto& reports_per_receiver : received_report_blocks_s_)
      for (const auto& report : reports_per_receiver.second)
        receive_blocks->push_back(report.second.report_block());
  }
  return 0;
...
std::vector<ReportBlockData> RTCPReceiver::GetLatestReportBlockData(int pathid) const {
...
bool RTCPReceiver::ParseCompoundPacket(rtc::ArrayView<const uint8_t> packet,
  ...
  for (const uint8_t* next_block = packet.begin(); next_block != packet.end();
    ...
    // //RTC_LOG(INFO)<<"sandystats path id of RTCP packet received "<<rtcp_block.pathid()<<" type "<< 
    // rtcp_block.type();
    //sandy: Let us first get the pathid
    packet_information->pathid=rtcp_block.pathid();
    ...
    switch (rtcp_block.type()) {
      ...
      case rtcp::ReceiverReport::kPacketType:
        ////RTC_LOG(INFO)<<"sandystats hangle the Rececived report\n";
      ...
      case rtcp::Rtpfb::kPacketType:
        switch (rtcp_block.fmt()) {
          ...
          case rtcp::TransportFeedback::kFeedbackMessageType:
           // //RTC_LOG(INFO)<<"sandystats handle the TransportFeedback\n";
...
void RTCPReceiver::HandleSenderReport(const CommonHeader& rtcp_block,
  ...
  if (remote_ssrc_ == remote_ssrc) {
    ...
    //RTC_LOG(INFO)<<"sandy received the sender report "<<packet_information->pathid<<"\n";
    packet_information->packet_type_flags |= kRtcpSr;
    if(rtcp_block.pathid()!=2){
      remote_sender_ntp_time_p_ = sender_report.ntp();
      remote_sender_rtp_time_p_ = sender_report.rtp_timestamp();
      last_received_sr_ntp_p_ = TimeMicrosToNtp(clock_->TimeInMicroseconds());
    }else{
      remote_sender_ntp_time_s_ = sender_report.ntp();
      remote_sender_rtp_time_s_ = sender_report.rtp_timestamp();
      last_received_sr_ntp_s_ = TimeMicrosToNtp(clock_->TimeInMicroseconds());
    }
...
void RTCPReceiver::HandleReceiverReport(const CommonHeader& rtcp_block,
  ...
  packet_information->packet_type_flags |= kRtcpRr;
  //sandy: Rececived RTCP report
  //RTC_LOG(INFO)<<"sandy received the received report "<<packet_information->pathid<<"\n";
...
void RTCPReceiver::HandleReportBlock(const ReportBlock& report_block,
  ...
  ////RTC_LOG(INFO)<<"sandy: The path id in the handling of the reports "<<packet_information->pathid<<"\n";
  if (registered_ssrcs_.count(report_block.source_ssrc()) == 0)
    return;

  const Timestamp now = clock_->CurrentTime();
  if(packet_information->pathid!=2)
    last_received_rb_ms_p_ = now.ms();
  else
    last_received_rb_ms_s_ = now.ms();

  ReportBlockData* report_block_data =nullptr;
  if(packet_information->pathid!=2){
    report_block_data=&received_report_blocks_p_[report_block.source_ssrc()][remote_ssrc];
  } else if(packet_information->pathid==2){
    report_block_data=&received_report_blocks_s_[report_block.source_ssrc()][remote_ssrc];
  }
  ...
  // if(packet_information->pathid!=2)
    //RTC_LOG(INFO)<<"sandy primary loss report: loss  cumulative_lost_signed "<<rtcp_report_block.packets_lost;
  // else
    //RTC_LOG(INFO)<<"sandy secondary loss report: loss  cumulative_lost_signed "<<rtcp_report_block.packets_lost;
  ...
  //sandy: Get the original ACK number
  rtcp_report_block.extended_highest_sequence_number_p =
      report_block.extended_high_seq_num_p();
  // //RTC_LOG(INFO)<<"sandy the original sequence number received is "<<report_block.extended_high_seq_num_p()<< 
  // "sequence number for same primary path "<<report_block.extended_high_seq_num()<<"sandy the RTCP sequence number "<<report_block.rtcp_seq()<<"\n";
  ...
  if (send_time_ntp != 0) {
    ...
    // //RTC_LOG(INFO)<<"sandy RTT computation receive time "<<now.ms();

    // RTT in 1/(2^16) seconds.

    uint32_t rtt_ntp = receive_time_ntp - delay_ntp - send_time_ntp;
    // Convert to 1/1000 seconds (milliseconds).
    rtt_ms = CompactNtpRttToMs(rtt_ntp);//sandy: RTT computation.
    // //RTC_LOG(INFO)<<"sandy1 RTT computation: delay "<<delay_ntp<<" delay ms "<< CompactNtpRttToMs(delay_ntp)<< 
    // " last SR time "<<send_time_ntp<<" receive_time  "<<  
    // now.ms()<<"RTT in NTP "<<rtt_ntp<<" RTT in ms "<<rtt_ms;
...
bool RTCPReceiver::RtcpRrTimeout(int pathid) {
  rtc::CritScope lock(&rtcp_receiver_lock_);

  if(pathid!=2){
    if (last_received_rb_ms_p_ == 0)
      return false;

    int64_t time_out_ms = kRrTimeoutIntervals * report_interval_ms_;
    if (clock_->TimeInMilliseconds() > last_received_rb_ms_p_ + time_out_ms) {
    ...
    if (last_received_rb_ms_s_ == 0)
      return false;

    int64_t time_out_ms = kRrTimeoutIntervals * report_interval_ms_;
    if (clock_->TimeInMilliseconds() > last_received_rb_ms_s_ + time_out_ms) {
      // Reset the timer to only trigger one log.
      last_received_rb_ms_s_ = 0;
      return true;
    }
...
void RTCPReceiver::HandleSdes(const CommonHeader& rtcp_block,
  ...
  // //RTC_LOG(INFO)<<"sandy the sdes message is received\n";
...
void RTCPReceiver::HandleBye(const CommonHeader& rtcp_block) {
  ...
  // Clear our lists.
  for (auto& reports_per_receiver : received_report_blocks_p_)
    reports_per_receiver.second.erase(bye.sender_ssrc());
  for (auto& reports_per_receiver : received_report_blocks_s_)
    reports_per_receiver.second.erase(bye.sender_ssrc());
...
void RTCPReceiver::HandleXr(const CommonHeader& rtcp_block,
  ...
  // //RTC_LOG(INFO)<<"sandy ExtendedReports received\n";
...
void RTCPReceiver::HandleXrDlrrReportBlock(const rtcp::ReceiveTimeInfo& rti) {//sandy:This is never called
  ...
  // //RTC_LOG(INFO)<<"sandystats xr_rr_rtt_ms_ is enabled\n";
...
void RTCPReceiver::HandleTransportFeedback(
  ...
  ////RTC_LOG(INFO)<<"sandystats path id of RTCP packet received\t"<<rtcp_block.pathid()<<"\n";
  std::unique_ptr<rtcp::TransportFeedback> transport_feedback(
      new rtcp::TransportFeedback(rtcp_block.pathid()));

  rtcp::TransportFeedback *transport_feedback_p(
      new rtcp::TransportFeedback(rtcp_block.pathid()));
  ...
  transport_feedback_p->Parse(rtcp_block);

  //RTC_DLOG(LS_ERROR)<<"sandytferror received feed back starting at "<<transport_feedback->GetBaseSequence();
  packet_information->pathid=rtcp_block.pathid();
  packet_information->packet_type_flags |= kRtcpTransportFeedback;
  packet_information->transport_feedback = std::move(transport_feedback);
  if(packet_information->pathid==2 && transport_feedback_p){
    MpCurrentSecondFeedback=transport_feedback_p;
  }
}
...
void RTCPReceiver::TriggerCallbacksFromRtcpPacket(
  ...
  ////RTC_LOG(INFO)<<"sandystats received RTCP packet\n";
  ...
  //sandy: Studying this now
  if (!receiver_only_ && (packet_information.packet_type_flags & kRtcpNack)) {
    if (!packet_information.nack_sequence_numbers.empty()) {
      RTC_LOG(LS_VERBOSE) << "Incoming NACK length: "
                          << packet_information.nack_sequence_numbers.size();
      rtp_rtcp_->OnReceivedNack(packet_information.nack_sequence_numbers,packet_information.pathid);//sandy: Added pathid for rececived report
    }
  }
  ...
  //sandy: Received loss notification RTCP
  ...
  if (rtcp_bandwidth_observer_) {
    ...
    if (packet_information.packet_type_flags & kRtcpRemb) {
       ////RTC_LOG(INFO)<<"sandytfb received remote bitrate information\n"; //sandy:This is never received
      RTC_LOG(LS_VERBOSE)
          << "Incoming REMB: "
          << packet_information.receiver_estimated_max_bitrate_bps;
    ...
    //sandy: Done with this implementation
    if ((packet_information.packet_type_flags & kRtcpSr) ||
        (packet_information.packet_type_flags & kRtcpRr)) {
      int64_t now_ms = clock_->TimeInMilliseconds();

      ////RTC_LOG(INFO)<<"sandystats calling the received rtcp report_block\n";
    //sandy:rtcp_bandwidth_observer_:rtp_transport_controller_send.cc
    //UPdates the google congestion controller with RTT and loss information
      rtcp_bandwidth_observer_->OnReceivedRtcpReceiverReport(
          packet_information.report_blocks, packet_information.rtt_ms, now_ms,packet_information.pathid);
          //packet_information.report_blocks, 200, now_ms,packet_information.pathid);
    }
  }
  //sandy: Done with this component
  if ((packet_information.packet_type_flags & kRtcpSr) ||
      (packet_information.packet_type_flags & kRtcpRr)) {
  ...
  if (transport_feedback_observer_ &&
      (packet_information.packet_type_flags & kRtcpTransportFeedback)) {
    //sandy: This is transport feed back
    ////RTC_LOG(INFO)<<"sandytfb received packet information of feedback\n";//This is always received.
    uint32_t media_source_ssrc =
        packet_information.transport_feedback->media_ssrc();
    if (media_source_ssrc == local_ssrc ||
        registered_ssrcs.find(media_source_ssrc) != registered_ssrcs.end()) {
     // //RTC_LOG(INFO)<<"sandystats calling the transport_feedback\n";
      //sandy: You need to delay the feedback from second path.
  ...
  if (!receiver_only_) {
    if (stats_callback_) {
      for (const auto& report_block : packet_information.report_blocks) {
        ...
        //sandy: Save the original sequence number
        stats.extended_highest_sequence_number_p =
            report_block.extended_highest_sequence_number_p;
