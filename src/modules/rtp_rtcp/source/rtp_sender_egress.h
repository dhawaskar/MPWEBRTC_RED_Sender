...
class RtpSenderEgress {
 public:
  ...
  RtpSendRates GetSendRates(int pathid) const RTC_LOCKS_EXCLUDED(lock_);
  RtpSendRates GetSendRatesOverall() const RTC_LOCKS_EXCLUDED(lock_);
 ...
 private:
  ...
  RtpSendRates GetSendRatesLocked(int pathid) const RTC_EXCLUSIVE_LOCKS_REQUIRED(lock_);
  RtpSendRates GetSendRatesLockedOverall() const RTC_EXCLUSIVE_LOCKS_REQUIRED(lock_);
  ...
  // uint16_t sequence_number_p_=1 ;
  // uint16_t sequence_number_s_=1 ;
  ...
  RtpPacketHistory* const packet_history_p_;
  RtpPacketHistory* const packet_history_s_;
  ...
  StreamDataCounters rtp_stats_p_ RTC_GUARDED_BY(lock_);
  StreamDataCounters rtp_stats_s_ RTC_GUARDED_BY(lock_);
  StreamDataCounters rtx_rtp_stats_ RTC_GUARDED_BY(lock_);
  StreamDataCounters rtx_rtp_stats_p_ RTC_GUARDED_BY(lock_);
  StreamDataCounters rtx_rtp_stats_s_ RTC_GUARDED_BY(lock_);
  ...
  std::vector<RateStatistics> send_rates_p_ RTC_GUARDED_BY(lock_);
  std::vector<RateStatistics> send_rates_s_ RTC_GUARDED_BY(lock_);
