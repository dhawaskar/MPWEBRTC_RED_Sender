...
class ModuleRtpRtcpImpl2 final : public RtpRtcpInterface,
 ...
 public:
  ...
  RTCPSender::FeedbackState GetFeedbackStatePrimary();
  RTCPSender::FeedbackState GetFeedbackStateSecondary();
  ...
  void OnPacketsAcknowledged(
      rtc::ArrayView<const uint16_t> sequence_numbers,int pathid) override;
  ...
  int32_t RemoteNTP(uint32_t* received_ntp_secs,
                    ...
                    uint32_t* rtcp_timestamp,int pathid) const override;
  ...
  int32_t RTT(uint32_t remote_ssrc,
              ...
              int64_t* avg_rtt,
              int64_t* min_rtt,
              int64_t* max_rtt,int pathid) const override;
  ...
  void GetSendStreamDataCounters(
      ...
      StreamDataCounters* rtx_counters,int pathid) const override;
  ...
  std::vector<ReportBlockData> GetLatestReportBlockData(int pathid) const override;
  ...
  void SendNack(const std::vector<uint16_t>& sequence_numbers,int pathid) override;

  // Store the sent packets, needed to answer to a negative acknowledgment
  // requests.
  void SetStorePacketsStatus(bool enable, uint16_t number_to_store) override;

  bool StorePackets(int pathid) const override;

  void SendCombinedRtcpPacket(
      std::vector<std::unique_ptr<rtcp::RtcpPacket>> rtcp_packets,int pathid) override;
  ...
  void OnReceivedNack(
      const std::vector<uint16_t>& nack_sequence_numbers,int pathid) override;
 ...
 private:
  ...
  struct RtpSenderContext {
    ...
    //sandy Creating two history handlers
    // Storage of packets, for retransmissions and padding, if applicable.
    RtpPacketHistory packet_history_p;
    RtpPacketHistory packet_history_s;
  ...
  void set_rtt_ms(int64_t rtt_ms,int pathid);
  void rtt_ms(int pathid,int64_t *rtt) const;
  ...
  int64_t last_rtt_process_time_p_;
  int64_t last_rtt_process_time_s_;
  int64_t next_process_time_p_;
  int64_t next_process_time_s_;
  ...
  rtc::CriticalSection critical_section_rtt_p_;
  int64_t rtt_ms_p_;
  rtc::CriticalSection critical_section_rtt_s_;
  int64_t rtt_ms_s_;
};
