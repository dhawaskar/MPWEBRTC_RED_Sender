 ...
 public:
  ...
  size_t SendPackets(size_t max_payload_length,int pathid) {
    ...
    Build(max_payload_length, [&](rtc::ArrayView<const uint8_t> packet) {
      //RTC_LOG(INFO)<<"sandyrtcp before packet sent 1\n";
      if (transport_->SendRtcp(packet.data(), packet.size(),pathid)) {
        //RTC_LOG(INFO)<<"sandyrtcp after packet sent 1\n";
...
RTCPSender::RTCPSender(const RtpRtcpInterface::Configuration& config)
    : audio_(config.audio),
      ...
      sending_(false),
      next_time_to_send_rtcp_p_(0),
      next_time_to_send_rtcp_s_(0),
      timestamp_offset_(0),
...
void RTCPSender::SetRTCPStatus(RtcpMode new_method) {//sandy: studying this
  rtc::CritScope lock(&critical_section_rtcp_sender_);

  if (method_ == RtcpMode::kOff && new_method != RtcpMode::kOff) {
    // When switching on, reschedule the next packet
    next_time_to_send_rtcp_p_ =
        clock_->TimeInMilliseconds() + (report_interval_ms_ / 2);
    next_time_to_send_rtcp_s_ =
        clock_->TimeInMilliseconds() + (report_interval_ms_ / 2);
  }
  method_ = new_method;
}
...
int32_t RTCPSender::SetSendingStatus(const FeedbackState& feedback_state,
  ...
  bool sendRTCPBye = false;
  {
    ...
    RTC_LOG(INFO)<<"sandy the sending status is set to "<<sending<<"\n";
    sending_ = sending;
  }
  if (sendRTCPBye)
...
void RTCPSender::SetRemb(int64_t bitrate_bps, std::vector<uint32_t> ssrcs) {
  ...
  next_time_to_send_rtcp_p_ = clock_->TimeInMilliseconds();
  next_time_to_send_rtcp_s_ = clock_->TimeInMilliseconds();
}
...
bool RTCPSender::TimeToSendRTCPReport(bool sendKeyframeBeforeRTP,int pathid) const {
  /*
  ...
  */
  ...
  if (!audio_ && sendKeyframeBeforeRTP) {
    ...
    now += RTCP_SEND_BEFORE_KEY_FRAME_MS;//100ms
  }

  if(pathid!=2){//sandy: Primary path
    if (now >= next_time_to_send_rtcp_p_) {
      return true;
    } else if (now < 0x0000ffff &&
               next_time_to_send_rtcp_p_ > 0xffff0000) {  // 65 sec margin
      // wrap
      return true;
    }
  }else{//sandy: Secondary path
    if (now >= next_time_to_send_rtcp_s_) {
      return true;
    } else if (now < 0x0000ffff &&
               next_time_to_send_rtcp_s_ > 0xffff0000) {  // 65 sec margin
      // wrap
      return true;
    }
  }
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildSR(const RtcpContext& ctx,const int pathid) {
  ...
  report->SetPathId(pathid);//send report to specific path 
  report->SetSenderSsrc(ssrc_);
  stime=TimeMicrosToNtp(ctx.now_us_);
  report->SetNtp(stime);
  // RTC_LOG(INFO)<<"sandy1 RTT computation SR time : NTP"<<CompactNtp(stime)<<" in ms "<<clock_->TimeInMicroseconds()<<" ctx time: "<< 
  // ctx.now_us_;
  report->SetRtpTimestamp(rtp_timestamp);
  report->SetPacketCount(ctx.feedback_state_.packets_sent);
  report->SetOctetCount(ctx.feedback_state_.media_bytes_sent);
  if(pathid!=2){
    report->SetReportBlocks(MpPrimaryCreateReportBlocks(ctx.feedback_state_));
  }else{
    report->SetReportBlocks(MpSecondaryCreateReportBlocks(ctx.feedback_state_));
  }
  //report->SetReportBlocks(CreateReportBlocks(ctx.feedback_state_));//sandy: Add both primary and secondary here
  //sandy: Here is where you need to add your RTCP extension header.

  return std::unique_ptr<rtcp::RtcpPacket>(report);
}

std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildSDES(//sandy: sent
    const RtcpContext& ctx,const int pathid) {
  // RTC_LOG(INFO)<<"sandyrtt: Building the SDES report "<<pathid<<"\n";
  size_t length_cname = cname_.length();
  RTC_CHECK_LT(length_cname, RTCP_CNAME_SIZE);

  rtcp::Sdes* sdes = new rtcp::Sdes();
  sdes->SetPathId(pathid);
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildPLI(const RtcpContext& ctx,const int pathid) {//sandy:not sent
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending PLI path= "<<pathid<<"\n";//sandy: Sending the picture loss indication and should be done via primary path
  rtcp::Pli* pli = new rtcp::Pli();
  pli->SetPathId(pathid);//send pli always via praimry path
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildFIR(const RtcpContext& ctx,const int pathid) {//sandy: not sent
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending FIR "<<pathid<<"\n";
  ++sequence_number_fir_;

  rtcp::Fir* fir = new rtcp::Fir();
  fir->SetPathId(pathid);//sandy: Sent FIR always via primary path
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildREMB(//sandy: not sent
    const RtcpContext& ctx,const int pathid) {

  // RTC_LOG(INFO)<<"sandyrtt building the REMB\n";
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildTMMBR(//sandy: not sent
    const RtcpContext& ctx,const int pathid) {
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending TMMBR\n";
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildTMMBN(//sandy: not sent
    const RtcpContext& ctx,const int pathid) {
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending TMBN\n";
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildAPP(const RtcpContext& ctx,const int pathid) {
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildLossNotification(//sandy: not sent
    const RtcpContext& ctx,const int pathid) {
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending loss "<<pathid<<"\n";
  ...
  loss_notification->SetPathId(pathid);//sandy: setting the path id as primary
  loss_notification->SetSenderSsrc(ssrc_);
  loss_notification->SetMediaSsrc(remote_ssrc_);
  return std::move(loss_notification);
}

std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildNACK(//sandy: This is sent
    const RtcpContext& ctx,const int pathid) {
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending NACK for "<<pathid<<"\n";
  
  rtcp::Nack* nack = new rtcp::Nack();
  nack->SetPathId(pathid);//sandy: This is how you set the path
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildBYE(const RtcpContext& ctx,const int pathid) {
  //sandy: This is sent
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending BYE\n";
...
std::unique_ptr<rtcp::RtcpPacket> RTCPSender::BuildExtendedReports(//sandy:Dont worry
    const RtcpContext& ctx,const int pathid) {
  // RTC_LOG(INFO)<<"sandyrtt: Building the sending extebdedreport\n";
...
int32_t RTCPSender::SendRTCP(const FeedbackState& feedback_state,
                             ...
                             int32_t nack_size,
                             const uint16_t* nack_list,int pathid) {
  //RTC_LOG(INFO)<<"sandyrtt sending RTCP report: "<<packetType<<"\n";
  // RTC_LOG(INFO)<<"sandyrtcp: Sending RTCP of type: "<<packetType<<":"<<pathid<<"\n";
  return SendCompoundRTCP(
      feedback_state, std::set<RTCPPacketType>(&packetType, &packetType + 1),
      nack_size, nack_list, pathid);
}
...
int32_t RTCPSender::SendCompoundRTCP(
...

  {
    ...
    auto result = ComputeCompoundRTCPPacket(feedback_state, packet_types,
                                            nack_size, nack_list, &container,pathid);
    if (result) {
      return *result;
    }
  ...
  size_t bytes_sent = container.SendPackets(max_packet_size,pathid);
  return bytes_sent == 0 ? -1 : 0;
}

int32_t RTCPSender::SendCompoundRTCPLocked(//sandy: This is only used to send the loss notification from receiver to sender. Hence the path id must always be set as 1 here
    const FeedbackState& feedback_state,
    const std::set<RTCPPacketType>& packet_types,
    int32_t nack_size,
    const uint16_t* nack_list) {
  PacketContainer container(transport_, event_log_);
  auto result = ComputeCompoundRTCPPacket(feedback_state, packet_types,
                                          nack_size, nack_list, &container,1);//sandy: should always be 1
  if (result) {
    return *result;
  }
  size_t bytes_sent = container.SendPackets(max_packet_size_,1);//sandy: should always be 1
  return bytes_sent == 0 ? -1 : 0;
}
...
absl::optional<int32_t> RTCPSender::ComputeCompoundRTCPPacket(
    ...
    int32_t nack_size,
    const uint16_t* nack_list,
    rtcp::CompoundPacket* out_packet,int pathid) {
  ...
  PrepareReport(feedback_state,pathid);
  ...
  //RTC_LOG(INFO)<<"sandy: Building the packets of each type\n";
  while (it != report_flags_.end()) {
    ...
    if (builder_it == builders_.end()) {
      ...
      std::unique_ptr<rtcp::RtcpPacket> packet = (this->*func)(context,pathid);
      if (packet == nullptr)
        return -1;
...
void RTCPSender::PrepareReport(const FeedbackState& feedback_state,int pathid) {
  ...
  if (IsFlagPresent(kRtcpSr) || IsFlagPresent(kRtcpRr)) {
    ...
    if (generate_report){
      //RTC_LOG(INFO)<<"sandy check the sending_ "<<sending_<<"\n";
      SetFlag(sending_ ? kRtcpSr : kRtcpRr, true);
    }
  }

  if (IsFlagPresent(kRtcpSr) || (IsFlagPresent(kRtcpRr) && !cname_.empty())){
    //RTC_LOG(INFO)<<"sandy: the sdes is sent either during the sender report or receiver report\n";
  ...
  if (generate_report) {
    ...
    if (!audio_ && sending_) {
      ...
      if (send_bitrate_kbit != 0) {
        ...
        //min_interval_ms = std::max(min_interval_ms,50);//sandy: The sending right migh be larger in mp-WebRTC due to 8 bytes of extra header
        min_interval_ms = std::min(min_interval_ms, report_interval_ms_);
      }
    }
    // RTC_LOG(INFO)<<"sandy the minimum RTCP SR report interval "<<min_interval_ms;
    // The interval between RTCP packets is varied randomly over the
    ...
    if(pathid!=2){
      //next_time_to_send_rtcp_p_ = clock_->TimeInMilliseconds() + 0;
      next_time_to_send_rtcp_p_ = clock_->TimeInMilliseconds() + time_to_next;
    }
    else{
      next_time_to_send_rtcp_s_ = clock_->TimeInMilliseconds() + time_to_next;
      //next_time_to_send_rtcp_s_ = clock_->TimeInMilliseconds() + 0;
    }
...
//sandy:MpwebRTC
std::vector<rtcp::ReportBlock> RTCPSender::MpPrimaryCreateReportBlocks(
    const FeedbackState& feedback_state) {

  //RTC_LOG(INFO)<<"rtcpsandy:Creating the report block\t"<<"\n";
  ...
  // streams.
  result = receive_statistics_->RtcpReportBlocks(RTCP_MAX_REPORT_BLOCKS,1);//pathid
  //sandy: Mp WebRTC calling it with path id 1 gets you primary stats and 2 secondary else default stats.
...
//sandy:MpwebRTC secondary needs modifcation or yet to implement.
std::vector<rtcp::ReportBlock> RTCPSender::MpSecondaryCreateReportBlocks(
    const FeedbackState& feedback_state) {
  ...
  // compound rtcp packet when single rtcp module is used for multiple media
  // streams.
  result = receive_statistics_->RtcpReportBlocks(RTCP_MAX_REPORT_BLOCKS,2);//pathid
  //sandy: Mp WebRTC calling it with path id 1 gets you primary stats and 2 secondary else default stats.
...
//sandy: This function only sends the Transport Feed Back messages.
void RTCPSender::SendCombinedRtcpPacket(
  ...
  auto callback = [&](rtc::ArrayView<const uint8_t> packet) {
    //RTC_LOG(INFO)<<"sandyrtcp before packet sent 3\n";
    if (transport_->SendRtcp(packet.data(), packet.size(),pathid)) {
      //RTC_LOG(INFO)<<"sandyrtcp before packet sent 3\n";
      if (event_log_)
        event_log_->Log(std::make_unique<RtcEventRtcpPacketOutgoing>(packet));
 ...
 // RTC_LOG(INFO)<<"sandyrtt Sending the RTCP packets\n";
  sender.Send(pathid);
}
