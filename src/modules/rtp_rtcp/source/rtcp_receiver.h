...
class RTCPReceiver final {
 public:
  class ModuleRtpRtcp {
   public:
    ...
    virtual void OnReceivedNack(
        const std::vector<uint16_t>& nack_sequence_numbers,int pathid=-1) = 0;
  ...
  int64_t LastReceivedReportBlockMs(int pathid) const;
  ...
  bool GetAndResetXrRrRtt(int64_t* rtt_ms,int pathid);

  // Get statistics.
  int32_t StatisticsReceived(std::vector<RTCPReportBlock>* receiveBlocks,int pathid) const;
  // A snapshot of Report Blocks with additional data of interest to statistics.
  // Within this list, the sender-source SSRC pair is unique and per-pair the
  // ReportBlockData represents the latest Report Block that was received for
  // that pair.
  std::vector<ReportBlockData> GetLatestReportBlockData(int pathid) const;

  // Returns true if we haven't received an RTCP RR for several RTCP
  // intervals, but only triggers true once.
  bool RtcpRrTimeout(int pathid);
 ...
 private:
  ...
  NtpTime remote_sender_ntp_time_p_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  uint32_t remote_sender_rtp_time_p_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  // When did we receive the last send report.
  NtpTime last_received_sr_ntp_p_ RTC_GUARDED_BY(rtcp_receiver_lock_);

  NtpTime remote_sender_ntp_time_s_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  uint32_t remote_sender_rtp_time_s_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  // When did we receive the last send report.
  NtpTime last_received_sr_ntp_s_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  ...
  std::map<uint32_t, TmmbrInformation> tmmbr_infos_
      RTC_GUARDED_BY(rtcp_receiver_lock_);
  //ReportBlockMap received_report_blocks_ RTC_GUARDED_BY(rtcp_receiver_lock_);//sandy:Primary report blocks
  ReportBlockMap received_report_blocks_p_ RTC_GUARDED_BY(rtcp_receiver_lock_);//sandy:Primary report blocks
  ReportBlockMap received_report_blocks_s_ RTC_GUARDED_BY(rtcp_receiver_lock_);//sandy:secondary report block
  ...
  int64_t last_received_rb_ms_p_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  int64_t last_received_rb_ms_s_ RTC_GUARDED_BY(rtcp_receiver_lock_);
