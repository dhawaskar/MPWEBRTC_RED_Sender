...
class RTPSender {
 public:
  ...
  void OnReceivedNack(const std::vector<uint16_t>& nack_sequence_numbers,
                      int64_t avg_rtt,int pathid) RTC_LOCKS_EXCLUDED(send_critsect_);

  int32_t ReSendPacket(uint16_t packet_id,int pathid) RTC_LOCKS_EXCLUDED(send_critsect_);
  int32_t ReSendPacketPrimary(uint16_t packet_id,int pathid) RTC_LOCKS_EXCLUDED(send_critsect_);//sandy
  int32_t ReSendPacketSecondary(uint16_t packet_id,int pathid) RTC_LOCKS_EXCLUDED(send_critsect_);//sandy
