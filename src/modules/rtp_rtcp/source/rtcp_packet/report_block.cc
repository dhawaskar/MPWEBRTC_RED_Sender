...
bool ReportBlock::Parse(const uint8_t* buffer, size_t length) {
  ...
  extended_high_seq_num_p_ = ByteReader<uint32_t>::ReadBigEndian(&buffer[24]);//sandy: save the original sequence number
  return true;
}
...
void ReportBlock::Create(uint8_t* buffer) const {
  ...
  ByteWriter<uint32_t>::WriteBigEndian(&buffer[24], extended_high_seq_num());//sandy: save the original sequence number
}
