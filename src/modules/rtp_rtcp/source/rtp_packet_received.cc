...
void RtpPacketReceived::GetHeader(RTPHeader* header) const {
  ...
  header->extension.hassandy=GetExtension<sandy>(&header->extension.sandy);
  header->extension.hasmpflowseqnum=GetExtension<MpFlowSeqNum>(&header->extension.mpflowseqnum);
  header->extension.hasMpTransportSequenceNumber = GetExtension<MpTransportSequenceNumber>(&header->extension.mptransportSequenceNumber);
